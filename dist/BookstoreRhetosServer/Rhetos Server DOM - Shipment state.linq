...
void Main()
{
    ...
    using (var container = new RhetosTestContainer(commitChanges: false)) // Use this parameter to COMMIT or ROLLBACK the data changes.
    {
		...
		repository.Bookstore.ComputeShipmentCurrentState.Load().Dump("ComputeShipmentCurrentState");
		repository.Bookstore.ShipmentCurrentState.Load().Dump("ShipmentCurrentState");
